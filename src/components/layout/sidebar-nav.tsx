...
const NAV_ITEMS = [
  ...
  { href: "/stocks", label: "Stock Command", icon: "📊" },
  ...
  return (
    <aside className="hidden md:flex flex-col w-56 min-h-screen bg-space-deeper border-r-2 border-[#2a2a4a] p-4 gap-2 fixed left-0 top-0 z-40">
      <div className="mb-6 px-2">
        <h1 className="font-pixel text-[10px] text-pixel-cyan leading-relaxed">
          PROJECT
          <br />
          MOONLANDER
        </h1>
        <p className="font-pixel-body text-xs text-pixel-purple mt-1">v1.0</p>
      </div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 font-pixel-body text-sm transition-all ${
                active
